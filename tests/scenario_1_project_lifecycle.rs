...
use deverp::domain::timeline::entity::{TimelineStatus, CreateMilestone, MilestoneStatus};
...
use deverp::infrastructure::repositories::timeline_repo::{PostgresTimelineRepository, PostgresMilestoneRepository};
...
async fn test_complete_project_lifecycle() {
    ...
    let task_service = TaskService::new(task_repo.clone(), task_dependency_repo.clone(), task_comment_repo.clone());
    ...
    let project = project_service.create_project(create_project)
        .await
    ...
    let task1 = task_service.create_task(create_test_task(project.id, "Design Database Schema"))
        .await
        .expect("Failed to create task 1");
    tasks.push(task1.clone());

    let task2 = task_service.create_task(create_test_task(project.id, "Implement API"))
        .await
        .expect("Failed to create task 2");
    tasks.push(task2.clone());

    let task3 = task_service.create_task(create_test_task(project.id, "Write Tests"))
        .await
        .expect("Failed to create task 3");
    tasks.push(task3.clone());

    let task4 = task_service.create_task(create_test_task(project.id, "Deploy to Production"))
        .await
    ...
    let timeline = timeline_service.create_timeline(create_timeline)
        .await
    ...
    let m1 = timeline_service.create_milestone(milestone1)
        .await
        .expect("Failed to create milestone 1");

    let m2 = timeline_service.create_milestone(milestone2)
        .await
    ...
    let resource1 = resource_service.create_resource(create_test_resource("PostgreSQL"))
        .await
        .expect("Failed to create resource 1");

    let resource2 = resource_service.create_resource(create_test_resource("Rust sqlx"))
        .await
    ...
    let project_resources = resource_service.get_project_resources(project.id)
        .await
    ...
    // Complete task 1
    task_service.update_task(deverp::domain::task::entity::UpdateTask {
        id: tasks[0].id,
        title: None,
        description: None,
    ...
    timeline_service.update_milestone(deverp::domain::timeline::entity::UpdateMilestone {
        id: m1.id,
        name: None,
        description: None,
        target_date: None,
    ...
    project_service.update_project(deverp::domain::project::entity::UpdateProject {
        id: project.id,
        name: None,
        description: None,
        code: None,
    ...
    let status_report = report_service.generate_project_status_report()
        .await
        .expect("Failed to generate status report");

    assert!(status_report.active_projects >= 1);

    let project_summaries = report_service.generate_project_summary()
        .await
        .expect("Failed to generate project summary");

    let project_summary = project_summaries.iter()
    ...
    let completed_project = project_service.update_project(deverp::domain::project::entity::UpdateProject {
        id: project.id,
        name: None,
        description: None,
        code: None,
    ...
    // Verify final state
    let final_summaries = report_service.generate_project_summary()
        .await
        .expect("Failed to generate final summary");

    let final_summary = final_summaries.iter()
