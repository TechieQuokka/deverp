...
async fn test_resource_management() {
    ...
    let postgres = resource_service.create_resource(create_test_resource("PostgreSQL"))
        .await
        .expect("Failed to create PostgreSQL resource");

    let rust = resource_service.create_resource(create_test_resource("Rust"))
        .await
        .expect("Failed to create Rust resource");

    let sqlx = resource_service.create_resource(create_test_resource("sqlx"))
        .await
        .expect("Failed to create sqlx resource");

    let tokio = resource_service.create_resource(create_test_resource("tokio"))
        .await
        .expect("Failed to create tokio resource");

    let docker = resource_service.create_resource(create_test_resource("Docker"))
        .await
    ...
    let project1 = project_service.create_project(create_test_project("Web API Project"))
        .await
        .expect("Failed to create project 1");

    let project2 = project_service.create_project(create_test_project("CLI Tool Project"))
        .await
        .expect("Failed to create project 2");

    let project3 = project_service.create_project(create_test_project("Microservice Project"))
        .await
    ...
    resource_service.link_resource_to_project(deverp::domain::resource::entity::LinkResourceToProject {
    ...
    for resource_id in [postgres.id, rust.id, sqlx.id, tokio.id, docker.id] {
        ...
        .await
        .expect(&format!("Failed to link resource {} to project 3", resource_id));
    }
    ...
    let project1_resources = resource_service.get_project_resources(project1.id)
        .await
        .expect("Failed to get project 1 resources");

    assert_eq!(project1_resources.len(), 4, "Project 1 should have 4 resources");

    let project2_resources = resource_service.get_project_resources(project2.id)
        .await
        .expect("Failed to get project 2 resources");

    assert_eq!(project2_resources.len(), 2, "Project 2 should have 2 resources");

    let project3_resources = resource_service.get_project_resources(project3.id)
        .await
        .expect("Failed to get project 3 resources");

    assert_eq!(project3_resources.len(), 5, "Project 3 should have 5 resources");

    // Step 5: Analyze resource utilization
    println!("Step 5: Analyzing resource utilization...");

    let all_resources = resource_service.list_resources(deverp::domain::resource::entity::ResourceFilter {
        resource_type: None,
        status: None,
        name_contains: None,
        tags: None,
    ...
    let rust_usage = resource_service.get_resource_usage(rust.id)
        .await
        .expect("Failed to get Rust usage");

    assert_eq!(rust_usage.total_projects, 3, "Rust should be used in 3 projects");

    // Check which projects use Docker (should be only project 3)
    let docker_usage = resource_service.get_resource_usage(docker.id)
        .await
        .expect("Failed to get Docker usage");

    assert_eq!(docker_usage.total_projects, 1, "Docker should be used in 1 project");

    // Step 6: Unlink a resource
    println!("Step 6: Unlinking resources...");

    resource_service.unlink_resource_from_project(project2.id, tokio.id)
        .await
        .expect("Failed to unlink tokio from project 2");

    let project2_resources_after = resource_service.get_project_resources(project2.id)
        .await
        .expect("Failed to get project 2 resources after unlink");

    assert_eq!(project2_resources_after.len(), 1, "Project 2 should have 1 resource after unlinking");

    // Step 7: Update resource status
    println!("Step 7: Updating resource status...");

    let deprecated_docker = resource_service.update_resource(deverp::domain::resource::entity::UpdateResource {
        id: docker.id,
        name: None,
        description: None,
        resource_type: None,
    ...
    let temp_resource = resource_service.create_resource(create_test_resource("Temp Resource"))
        .await
    ...
    resource_service.delete_resource(temp_resource.id)
        .await
        .expect("Failed to delete temp resource");

    // Verify it's no longer in the list
    let resources_after_delete = resource_service.list_resources(deverp::domain::resource::entity::ResourceFilter {
        resource_type: None,
        status: None,
        name_contains: None,
        tags: None,
    ...
    assert_eq!(resources_after_delete.len(), 5, "Should still have 5 non-deleted resources");

    // Step 9: Test filtering resources
    println!("Step 9: Testing resource filtering...");

    // Filter by status
    let active_resources = resource_service.list_resources(deverp::domain::resource::entity::ResourceFilter {
        resource_type: None,
    ...
    assert!(active_resources.len() >= 4, "Should have at least 4 active resources");

    let deprecated_resources = resource_service.list_resources(deverp::domain::resource::entity::ResourceFilter {
        resource_type: None,
        status: Some(ResourceStatus::Deprecated),
        name_contains: None,
        tags: None,
    ...
    assert_eq!(deprecated_resources.len(), 1, "Should have 1 deprecated resource (Docker)");
    ...
    let p1_resources = resource_service.get_project_resources(project1.id)
        .await
        .expect("Failed to get project 1 resources");
    assert_eq!(p1_resources.len(), 4, "Project 1 should have 4 resources");

    let p2_resources = resource_service.get_project_resources(project2.id)
        .await
        .expect("Failed to get project 2 resources");
...
async fn test_resource_search() {
    ...
    resource_service.create_resource(create_test_resource("React"))
        .await
        .expect("Failed to create React");

    resource_service.create_resource(create_test_resource("Vue"))
        .await
        .expect("Failed to create Vue");

    resource_service.create_resource(create_test_resource("Angular"))
        .await
