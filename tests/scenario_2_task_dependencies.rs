...
async fn test_task_dependency_management() {
    ...
    let dependency_repo = Arc::new(deverp::infrastructure::repositories::task_repo::PostgresTaskDependencyRepository::new(pool.clone()));
    let comment_repo = Arc::new(deverp::infrastructure::repositories::task_repo::PostgresTaskCommentRepository::new(pool.clone()));
    ...
    let project = project_service.create_project(create_test_project("Dependency Test Project"))
        .await
    ...
    let task_a = task_service.create_task(create_test_task(project.id, "Task A - Foundation"))
        .await
        .expect("Failed to create task A");

    let task_b = task_service.create_task(create_test_task(project.id, "Task B - Build on A"))
        .await
        .expect("Failed to create task B");

    let task_c = task_service.create_task(create_test_task(project.id, "Task C - Build on B"))
        .await
        .expect("Failed to create task C");

    let task_d = task_service.create_task(create_test_task(project.id, "Task D - Build on C"))
        .await
        .expect("Failed to create task D");

    let task_e = task_service.create_task(create_test_task(project.id, "Task E - Independent"))
        .await
    ...
    task_service.add_task_dependency(deverp::domain::task::entity::CreateTaskDependency {
        task_id: task_b.id,
    ...
    let task_b_deps = task_service.get_task_dependencies(task_b.id)
        .await
    ...
    let circular_result = task_service.add_task_dependency(deverp::domain::task::entity::CreateTaskDependency {
        task_id: task_a.id,
    ...
    assert!(circular_result.is_err(), "Circular dependency should have been rejected");
    ...
    let task_d_deps = task_service.get_task_dependencies(task_d.id)
        .await
        .expect("Failed to get task D dependencies");

    assert_eq!(task_d_deps.len(), 1, "Task D should have 1 direct dependency");
    ...
    task_service.remove_task_dependency(task_c.id, task_b.id)
        .await
        .expect("Failed to remove dependency C -> B");

    let task_c_deps_after = task_service.get_task_dependencies(task_c.id)
        .await
        .expect("Failed to get task C dependencies after removal");

    assert_eq!(task_c_deps_after.len(), 0, "Task C should have no dependencies after removal");

    // Step 6: Verify independent task has no dependencies
    println!("Step 6: Verifying independent task...");

    let task_e_deps = task_service.get_task_dependencies(task_e.id)
        .await
        .expect("Failed to get task E dependencies");

    assert_eq!(task_e_deps.len(), 0, "Independent task should have no dependencies");
    ...
    let task_d_deps_final = task_service.get_task_dependencies(task_d.id)
        .await
        .expect("Failed to get task D final dependencies");

    assert_eq!(task_d_deps_final.len(), 2, "Task D should have 2 dependencies");
    ...
    let task_f = task_service.create_task(create_test_task(project.id, "Task F"))
        .await
        .expect("Failed to create task F");

    let task_g = task_service.create_task(create_test_task(project.id, "Task G"))
        .await
    ...
    let task_f_deps = task_service.get_task_dependencies(task_f.id)
        .await
...
async fn test_self_dependency_rejection() {
    ...
    let project = project_service.create_project(create_test_project("Self Dependency Test"))
        .await
        .expect("Failed to create project");

    let task = task_service.create_task(create_test_task(project.id, "Self Task"))
        .await
        .expect("Failed to create task");

    // Attempt to create self-dependency
    let result = task_service.add_task_dependency(deverp::domain::task::entity::CreateTaskDependency {
        task_id: task.id,
...
async fn test_complex_circular_dependency() {
    ...
    let project = project_service.create_project(create_test_project("Complex Cycle Test"))
        .await
        .expect("Failed to create project");
    ...
    assert!(result.is_err(), "Circular dependency should be detected in long chain");
